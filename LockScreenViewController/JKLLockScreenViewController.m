...
- (void)viewDidLoad {
    ...
    switch (_lockScreenMode) {
        ...
        case LockScreenModeNew: {
            // [신규 모드]
            [self lsv_updateTitle:NSLocalizedStringFromTable(@"비밀번호 입력",    @"JKLockScreen", nil)
                         subtitle:NSLocalizedStringFromTable(@"비밀번호를 입력해 주세요", @"JKLockScreen", nil)];
            
            break;
        }
        case LockScreenModeChange:
            // [변경 모드]
            [self lsv_updateTitle:NSLocalizedStringFromTable(@"비밀번호 변경",    @"JKLockScreen", nil)
                         subtitle:NSLocalizedStringFromTable(@"새로운 비밀번호를 입력해주세요", @"JKLockScreen", nil)];
            break;
    }
...
- (BOOL)lsv_isPincodeValid:(NSString *)pincode {
    ...
    if (_lockScreenMode == LockScreenModeVerification) {
        printf("\n\n확인pincode -- %s\n", [pincode UTF8String]);
        if( [_confirmPincode isEqualToString:pincode])
        {
            ///키값
            [pwd setObject:pincode forKey:@"암호"]; //암호설정 on
            [pwd synchronize];
...
- (void)lsv_unlockScreenFailure {
    ...
    [_subtitleLabel setText:NSLocalizedStringFromTable(@"비밀번호가 틀렸습니다", @"JKLockScreen", nil)];
    ...
    dispatch_after(delayInSeconds, dispatch_get_main_queue(), ^(void){
        ...
        switch (_lockScreenMode) {
            ...
            case LockScreenModeNew: {
                // [신규 모드]
                [self lsv_updateTitle:NSLocalizedStringFromTable(@"비밀번호 입력",    @"JKLockScreen", nil)
                             subtitle:NSLocalizedStringFromTable(@"비밀번호를 입력해주세요", @"JKLockScreen", nil)];
            ...
            case LockScreenModeChange:
                // [변경 모드]
                [self lsv_updateTitle:NSLocalizedStringFromTable(@"비밀번호 변경",    @"JKLockScreen", nil)
                             subtitle:NSLocalizedStringFromTable(@"변경할 비밀번호를 입력해 주세요", @"JKLockScreen", nil)];
                break;
            default:
                break;
        }
...
- (void)lockScreenPincodeView:(JKLLockScreenPincodeView *)lockScreenPincodeView pincode:(NSString *)pincode {
    ...
    /*else if (_lockScreenMode == LockScreenModeNew) {
        ...
        // 재입력 타이틀로 전환
        [self lsv_updateTitle:NSLocalizedStringFromTable(@"암호 재입력",    @"JKLockScreen", nil)
                     subtitle:NSLocalizedStringFromTable(@"확인을 위해 한 번 더 입력해 주세요", @"JKLockScreen", nil)];
        //[self lsv_isPincodeValid:_confirmPincode];
