...
NSMutableArray* test();
NSString* Classify_Image(); //카테고리분류
NSMutableArray* RecognitionImage(); //해시태그
...
- (IBAction)getUrl:(id)sender {
  NSString* inference_result = Classify_Image();
...
static void GetTopN(
  ...
  std::greater<std::pair<float, int> > > top_result_pq;
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (top_result_pq.size() > num_results) top_result_pq.pop();
        }
...
NSString* Classify_Image() {
//    NSMutableArray *arrResult = [[NSArray alloc]init];
    ...
    NSString* network_path = FilePathForResourceName(@"quantized_stripped_images", @"pb");
    ...
    // Read the label list
    NSString* labels_path = FilePathForResourceName(@"images_retrained_labels", @"txt");
    ...
    // Read the "documnet/test3.jpg"
    //NSString* image_path = FilePathForResourceName(@"test3", @"jpg");
    AppDelegate *mAPP = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    NSString* image_path = mAPP.dimagePath;
    ...
    for (const auto& result : top_results) {
        ...
        ss << label_strings[index] << " " << confidence << " ";
    }
    ...
    NSMutableArray *arrResult = [[result componentsSeparatedByString:@" "]mutableCopy];
    
    //분류결과리턴!!!!
    if([[arrResult objectAtIndex:1] floatValue] > 0.75f)
    {
        NSString *category = [arrResult objectAtIndex:0];
        return category;
    }
    else
    {
        NSString *category = @"etc";
        return category;
    }
    return result;
}

NSMutableArray* RecognitionImage() {
    tensorflow::SessionOptions options;
